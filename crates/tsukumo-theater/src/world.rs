...
/// One soft-id actor on the workshop floor.
...
impl StageWorld {
    ...
    pub fn ensure_placeholder(&mut self, id: impl Into<ExecutorId>) {
        let id = id.into();
    ...
    fn resolve_id(&self, id: Option<&ExecutorId>) -> ExecutorId {
        id.cloned()
            .or_else(|| self.actors.first().map(|a| a.id.clone()))
            .unwrap_or_else(|| ExecutorId::new("anon"))
    }

    fn actor_mut(&mut self, id: &ExecutorId) -> &mut ActorSlot {
    ...
    pub fn apply(&mut self, event: &StageEvent) {
        match event {
            StageEvent::ActorPose { pose, executor_id } => {
                let id = self.resolve_id(executor_id.as_ref());
            ...
            StageEvent::Bubble { text, executor_id } => {
                let id = self.resolve_id(executor_id.as_ref());
                let actor = self.actor_mut(&id);
