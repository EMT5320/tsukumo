...
> 日期：2026-07-08
> 合并来源：`archive/DESIGN-A.md`（付丧神叙事主线）+ `archive/DESIGN-B.md`（事件驱动内核路线）+ 2026-07-07 讨论共识 + **2026-07-08 愿景收束讨论（定位/终态形态/演出节奏/注入协议）**
...
### 5.6 Cargo workspace 布局（草案）

```
tsukumo/
├── crates/
│   ├── tsukumo-kernel/    # L1: loop, events, tools, session, provider
│   ├── tsukumo-adapters/  # L1: ACP client / transcript watcher → KernelEvent（A1 里程碑起建）
...
边界纪律：kernel 禁止 import 上层、theater 只消费事件、adapters 只生产 KernelEvent。
...
舞台平时是余光里的鱼缸，只在三类"值得打扰"的事件上升级表现：**阻塞待审批、失败、完成**。演出密度三档（展开=剧场 / 折叠=精简 / 关闭=静默）在"用户手动切换"之外，叠加**事件驱动的注意力分级**——这正是竞品验证过的注意力管理效用的落点。
...
- 终态扩展方向：委托看板（多并行任务）进入大厅布局（UI 形态见开放问题）
...
复用 Loomstead 美术经验（见 §14）：风格关键词、色板、Prompt 模板、命名/manifest 组织。终端适配调整：强调高对比色块、粗轮廓、无渐变无 dithering、blocky shapes。
...
> 终态还有第二粒度角色：外部 runtime 的雇佣兵人设（见 §2.4，人设与真实 runtime 的绑定尺度见开放问题）。
...
> 备注（2026-07-08）："冒险者公会"隐喻与"多 agent 任务指挥所"这个真实工作流是同一个东西：委托看板 = 并行任务队列，队员出任务 = 后台 agent 跑分支，结算画面 = PR review。世界观不只是皮，是对新工作流的准确命名。
...
- 任务模式（命令式可靠执行）
- **A1 adapter spike**（M1 后尽早，见 §15）

### MVP 不做（留接口）
- 完整注入协议（§8：受管区块 / MCP 记忆服务 / 简报编译）—— A1 只做事件翻译
- 多 runtime 完整接入与观察级 watcher —— 架构预留 adapter 位
...
- Process Fidelity 评测 / 反事实回放等研究件 —— 不为求职叙事提前搬进 MVP（见 §18）
...
### 2026-07-08 已化解
- [x] 演出节奏 vs 效率的快进模式细节 → 由状态/叙事分层原则化解（§6），剩余细节归入导演器叙事剪辑规则
- [x] 项目终态形态（完整工具 / 增强层 / 指挥所） → 拍板"有灵魂的指挥所"（§2）
- [x] 注入与工具自带提示词/AGENTS.md 的冲突 → 注入协议四解法（§8）

### 仍开放
- [ ] 主搭档人格设定（90% 对话面向的角色，需专门讨论）
- [ ] soul/guise 两层设计是否最终采用（倾向是，待拍板）
- [ ] 角色阵容细节与命名
- [ ] 雇佣兵角色的人设与真实 runtime 的绑定尺度（品牌拟人化的分寸）
- [ ] 委托看板（多并行任务）的大厅 UI 形态
- [ ] A1 adapter 的具体通道选型（ACP vs headless stream-json，取决于届时 Claude Code 的 ACP 成熟度）
- [ ] 观察级 watcher 的转录格式覆盖范围
- [ ] MCP 记忆服务的工具面设计（recall/search 的参数 schema）
- [ ] 简报的容量上限与相关性过滤策略
