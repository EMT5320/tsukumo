...
**尚未形成标准产品、但也未证明归 tsukumo 独占的组合**：

- 跨 runtime 的用户状态、执行回执、来源与撤销语义目前分散在记忆服务、agent client、编排层和 observability 工具中；
- “养成数值来自真实执行数据”仍有叙事吸引力，但必须先证明这些数据会改善续接、纠错或留存；
- 拥有会话、per-person 状态和多 runtime 聚合都是可复制能力，单项或简单交集不能直接称为护城河。

**定位结论**：注意力管理与跨工具连续性都是入场券。候选差异化收窄为 runtime 无关、带可执行来源与撤销效果的可信交接；是否成为护城河由 §2.7 的强基线实验决定。
...
- **新物种"养成伙伴"入场**：Buddy（MCP 虚拟宠物，21 物种 / XP / 情绪 / 跨 client 持久，8 周 1 万+ clone）、CodePal（5 维情绪引擎桌宠）。**"竞品小人没有灵魂、不记得昨天"的论述部分失效**；其 progression 机制较轻，但仍构成真实替代品，不能用贬低式命名回避竞争；
...
├─ Host + normalized KernelEvent + runtime adapters ─┤
│ Claude / Codex / ACP / degraded transcript watcher │
├─ optional consumers ───────────────────────────────┤
│ TUI guild hall / inspector / replay / future panel │
└────────────────────────────────────────────────────┘
```
...
Process Fidelity 留下的商业纪律是：状态从真实流量派生仍只满足 integrity；它还需在迁移、恢复或依恋上产生可测收益，才能形成产品价值。
...
周末现实检验给出的结论更尖锐：Trellis 已经不是“Markdown + 自律”的弱基线。它包含项目规范与上下文注入，也可以通过 channel runtime 协调、观察和中断多模型 worker。用户用“共享制品 + 强模型 + 主动调度”在数天内完成了当前规模且质量良好的实现。跨工具协调、上下文连续和多模型切换已被轻量层覆盖，不能继续作为 tsukumo 护城河。

原先四条“轻量层结构性够不着”的断言全部降格为待测条件：

1. **事务性捕获**只有在低噪声、可操作并降低续接成本时才有价值；journal 不完整却没有阻止周末成功；
2. **拥有会话**是能力与入场券，编辑器、ACP client 和增强层 runtime 都可复制；
3. **per-person 状态**是作用域选择，容易被相邻产品扩展；
4. **聚合层抗吸收**没有天然保证，宿主也可以内置聚合与持久状态。

剩余可证伪假设：

> 自动跨 runtime 迁移的执行状态，如果同时带有来源、scope、ProjectionReceipt、行为 outcome、选择性 revoke 和后续投影效果，能够降低真实工作的续接、纠错与故障恢复成本。

### 证据阶梯

| 层级 | 能证明什么 | 不能证明什么 |
|---|---|---|
| 链路完整 | 实现可追溯、可回放 | 状态影响了行为或对用户有用 |
| removed-state 行为变化 | 状态进入了受控决策路径 | 任务更快、更好或更省心 |
...
养成数值必须来自可追溯的真实执行数据，并明确它代表使用、成功率还是关系事实。数据来源真实只满足 integrity；只有它影响未来体验或帮助主人理解系统时，才产生产品价值。映射曲线仍可前快后慢（对数 / 里程碑解锁），但不得用表现节奏掩盖无效指标。

Loomstead 的 Direct State Setter baseline 与 Process Fidelity 提供了“硬写状态 vs 经历派生状态”的完整性对照，也证明完整性不足以推出行为与用户价值。羁绊 / 熟练度可由事件 reducer 派生；是否保留到产品由后续效用和依恋验证决定。
...
搬**设计资产**（schema/机制/经验教训），不搬代码（Loomstead 是 Python，tsukumo 是 Rust）。2026-07-13 源码与研究档案复核后，Loomstead 留下**三件正向资产 + 一件更重要的负面资产**：

### 17.2 四件愿景级资产

**1. Trace 优先的可观测性 DNA —— "注意力管理"的学术前身**

Loomstead 终版定位是 Agent Behavior Observatory（`sourceEventIds` / `traceRefs` / `candidateScores` 证据链 + 反事实回放），核心问题 "Why did this agent choose this action?"。其 trace / event / eval 分层仍是 tsukumo 的工程 DNA。Tsukumo 把问题移到独立 runtime 边界：哪些用户状态被迁移、实际投影给谁、产生了什么动作、撤销后未来行为如何变化。

**2. Motivational Delegation —— 关系层的理论骨架**

Director 通过 `motivation_bias` / `opportunity_schedule` / `information_exposure` / `constraint_injection` 间接影响 NPC，留下“成长来自经历而非直接改数值”的优秀设计原则。Process Fidelity 的 promoted scenarios 后来没有稳定产生行为分化，因此该原则属于理论与机制资产，不能再表述为已验证的用户价值。

**3. Coding domain adapter 已验证 schema 可迁移**

Loomstead 跨域评测中 coding domain fixture 用同一套 GoalSpec / Intervention / Trace / Eval schema 跑出 40/40 通过、反事实工具选择变化率 0.762（55/55 确定性 seed）。它证明 schema 与 replay scaffold 可以迁到编码 fixture，也暴露 deterministic rule / toy scenario 的 claim 边界；它没有证明真实 coding workflow 的效率、质量或可用性收益。

**4. 证据完整 ≠ 行为分化 ≠ 用户价值 —— 最重要的负面资产**

`human_rating_pilot_gate.md` 关闭人工盲评的原因是 Memory / Relationship ablation 虽改变 evidence / integrity，promoted scenarios 的 `goalToolEvents` 仍一致。Loomstead 的教训必须直接约束 Tsukumo：source → state → receipt → outcome 全链只能算工程门；removed-state 改变工具参数只能算行为敏感；续接、纠错和恢复成本相对强基线下降，才是产品结果。

两项目的根本差异由系统边界和因变量定义：Loomstead 研究一个受控 runtime 内动作为什么被选择；Tsukumo 验证用户状态跨 Claude / Codex 等独立 runtime 后，是否降低真实工作的操作成本。若后一个因变量不变化，Tsukumo 不能用 Rust、TUI 或更完整 trace 包装成新的研究结论。
...
| Process Fidelity / Direct State Setter baseline | 证据阶梯与强基线设计 | 约束“trace 完整不能冒充用户价值”的 claim 边界 |
...
### Loomstead — 三件正向资产 + 一件负面资产
- Trace 可观测性 DNA、Motivational Delegation、coding adapter schema 迁移，以及“证据完整不等于用户价值”的失败教训（见 §17.2）
...
- 常见短板包括人格持久性、会话控制和成长机制，但这些只是候选切入点，未构成已证明的 tsukumo 护城河
